...
let
    ...
    @testset "sort" begin
        ...
        @test sort(rt, [2]) == RowTable(sort(df, [2]))
        @test sort(rt, [2], rev=true) == RowTable(sort(df, [2], rev=true))
        @test sort(rt, [2], rev=true) != RowTable(sort(df, [2]))
        @test sort(rt, [:d, :c], rev=true) == RowTable(sort(df, [:d, :c], rev=true))
    end
end
